...
import { faFileArchive, faUserTag } from '@fortawesome/free-solid-svg-icons';
...
export class ShareResourcesFormComponent {
  ...
  topic_document_error_message: string | undefined;
  ...
  share_resources_form: FormGroup = this.formbuilder.group({
    ...
    topic_document: ['', [Validators.required]],
  ...
  topic_document_error_alert_msg: boolean | undefined;
  ...
  ngOnInit(): void {
    ...
    const topic_document_error_control =
      this.share_resources_form.get('topic_document');
    topic_document_error_control?.valueChanges.subscribe((topic_document) => {
      this.validate_topic_document(topic_document_error_control as FormControl);
    });
  ...
  validate_topic_document(topic_document_error_control: FormControl): void {
    this.topic_document_error_message = ' ';
    this.topic_document_error_alert_msg = false;
    if (
      topic_document_error_control.invalid &&
      (topic_document_error_control.touched ||
        topic_document_error_control.dirty)
    ) {
      this.topic_document_error_alert_msg = true;
      if (topic_document_error_control.errors?.['required']) {
        this.topic_document_error_message = 'Topic Document is required';
      }
    }
  }
