...
export class AddSkillsAndSociallinksComponent {
  ...
  facebook_link_regex = /(?:https?:\/\/)?(?:www\.)?(mbasic.facebook|m\.facebook|facebook|fb)\.(com|me)\/(?:(?:\w\.)*#!\/)?(?:pages\/)?(?:[\w\-.]*\/)*([\w\-.]*)/ig
  twitter_link_regex = /(https:\/\/twitter.com\/(?![a-zA-Z0-9_]+\/)([a-zA-Z0-9_]+))/g;
  linkedin_link_regex = /^(http(s)?:\/\/)?([\w]+\.)?linkedin\.com\/(pub|in|profile)/gm;
  github_link_regex = /^(https?:\/\/)?(www\.)?github\.com\/[a-zA-Z0-9_]{1,25}$/igm;
  //Form builder
  social_link_form:FormGroup = this.formbuilder.group({
      facebook_link:['',[Validators.pattern(this.facebook_link_regex)]],
      twitter_link:['',[Validators.pattern(this.twitter_link_regex)]],
      linkedin_link:['',[Validators.pattern(this.linkedin_link_regex)]],
      github_link:['',[Validators.pattern(this.github_link_regex)]],
    },
  );
  ...
  //oninit function
  ngOnInit(): void {
    const facebook_link_error_control = this.social_link_form.get('facebook_link');
    ...
    const twitter_link_error_control = this.social_link_form.get('twitter_link');
    ...
    const linkedin_link_error_control = this.social_link_form.get('linkedin_link');
  ...
  validate_facebook_link(facebook_link_error_control:FormControl): void {
    ...
    if (facebook_link_error_control.invalid && (facebook_link_error_control.touched || facebook_link_error_control.dirty)) {
  ...
  validate_twitter_link(twitter_link_error_control:FormControl): void {
    ...
    if (twitter_link_error_control.invalid && (twitter_link_error_control.touched || twitter_link_error_control.dirty)) {
  ...
  validate_linkedin_link(linkedin_link_error_control:FormControl): void {
    ...
    if (linkedin_link_error_control.invalid && (linkedin_link_error_control.touched || linkedin_link_error_control.dirty)) {
  ...
  validate_github_link(github_link_error_control:FormControl): void {
    ...
    if (github_link_error_control.invalid && (github_link_error_control.touched || github_link_error_control.dirty)) {
  ...
  validate_skills(skills_error_control:FormControl): void {
    ...
    if (skills_error_control.invalid && (skills_error_control.touched || skills_error_control.dirty)) {
